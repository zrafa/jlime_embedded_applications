...
)
{
	...
	case FS_FAT32 :
		...
		if (disk_readp(buf, fs->fatbase + clst / 128UL, (WORD)((clst % 128UL) * 4UL), 4)) break;
...
)
{
	...
	*br = 0;
	...
	while (btr)	{									/* Repeat until all data transferred */
		if ((fs->fptr % 512UL) == 0) {				/* On the sector boundary? */
			if ((fs->fptr / 512UL % (DWORD) fs->csize) == 0) {	/* On the cluster boundary? */
		...
		rcnt = 512UL - (fs->fptr % 512UL);		/* Get partial sector data from sector buffer */
		if (rcnt > btr) rcnt = btr;
		dr = disk_readp(!buff ? 0 : rbuff, fs->dsect, (WORD)(fs->fptr % 512UL), rcnt);
		if (dr) goto fr_abort;
...
#define USE_CONTINUITIES
#ifdef USE_CONTINUITIES
typedef struct {
  DWORD begin;
  DWORD end;
} Continuity;

#define NumContinuities 4
Continuity continuities[NumContinuities];
char lastContinuity=0;

CLUST advance_continuity(CLUST pos, DWORD *offset) {
	FATFS *fs = FatFs;
  /* Find relevant continuity, or start new one */
  Continuity *cont = 0x0;
  for (int i=0; i < NumContinuities; ++i) {
    if (continuities[i].begin <= pos && continuities[i].end > pos) {
      cont = continuities + i;
      break;
    }
  }
  if (!cont) {
    cont = continuities + (++lastContinuity % NumContinuities);
    cont->begin = pos;
    cont->end = pos;
  }
  DWORD bcs = (DWORD) fs->csize * 512; /* cluster size in bytes */
  if (*offset > 0) {
    DWORD coffset = (*offset-1)/bcs;
    /* Are we within the range? */
    if (coffset + pos <= cont->end) {
      //printf(" (%lu, %lu) -> ", pos, *offset);
      *offset = *offset - coffset*bcs;
      //printf(" MATCH: [%lu - %lu] + %lu ", cont->begin, cont->end, *offset);
      return coffset + pos;
    }
    /* Advance as far as we know how to */
    *offset -= (cont->end - pos)*bcs; // this will be zero for new conts
    coffset = (*offset-1)/bcs;
    pos = cont->end;
  }
  /* single step through building up our continuity as we go */
  while (*offset > bcs) {
    CLUST npos = get_fat(pos);		/* Follow cluster chain */
    if (npos == pos+1) { 
      if (cont) {
        cont->end = npos;
      }
    } else { // discontinuity - stop our recording
      cont = 0x0;
    }
    if (npos <= 1 || npos >= fs->max_clust) return 0;
    pos = npos;
    *offset -= bcs;
  }
  return pos;
...
#ifdef USE_CONTINUITIES
    fs->fptr += ofs;
    clst = advance_continuity(clst, &ofs);
    fs->curr_clust = clst;
#else
