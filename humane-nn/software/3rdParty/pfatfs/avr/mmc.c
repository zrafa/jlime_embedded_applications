...
DSTATUS disk_initialize (void)
{
  ...
  // next two lines prevent re-init hang by some cards (not sure why this works) - from WaveHC - bcg'10
  SELECT(); 
	for (n = 255; n; n--) rcv_spi();	/* Dummy clocks */

	ty = 0;
  int r = 0;
  // From WaveHC - an additional long delay loop for CMD0 -bcg'10
  for (int retry = 60; (r != 1) && retry; retry--)
    r = send_cmd(CMD0, 0);
...
#ifdef SERIAL_OUT // I don't need this - bcg'10
