...
//START_EXTERN_C
//extern int get_direction_controler(void);
//extern int get_action_buttons(void);
//extern void I_InitAccelerometer(void);
//extern void I_CloseAccelerometer(void);
//END_EXTERN_C
...
int get_position(event_t *ev)
{
        ...
        int accelerometer_ywindow=60; // DeadZone in Pixels
...
static int S9xProcessSound (void *);
...
//	I_InitAccelerometer();
	...
	thread = SDL_CreateThread(S9xProcessSound, NULL);
	event_mutex = SDL_CreateMutex();

//	SDL_Thread *thread2 = NULL;

//	thread2 = SDL_CreateThread(runThread, NULL);
...
//	doneYet2 = 1;
//	SDL_WaitThread(thread2, NULL); // wait for it to die
...
//	SDL_mutexP ( event_mutex );	//lock before accessing mouseq
...
int kup=0, kdown=0, kleft=0, kright=0, kx=0, ky=0, ka=0, kb=0, ks=0, kt=0;
...
/* end for openmoko */
	...
	if (kup) val |= SNES_UP_MASK;
	if (kdown) val |= SNES_DOWN_MASK;
	if (kleft) val |= SNES_LEFT_MASK;
	if (kright) val |= SNES_RIGHT_MASK;
	if (kx) val |= SNES_X_MASK;
	if (ky) val |= SNES_Y_MASK;
	if (ka) val |= SNES_A_MASK;
	if (kb) val |= SNES_B_MASK;
	if (ks) {
		//printf("START\n");
		val |= SNES_START_MASK;
	};
	if (kt) val |= SNES_SELECT_MASK;
...
/*
	if (om_direction==1)
			...
			val |= SNES_UP_MASK | SNES_LEFT_MASK;
			...
			val |= SNES_DOWN_MASK | SNES_RIGHT_MASK;
	else if (om_direction==7)
			val |= SNES_RIGHT_MASK;
	else if (om_direction==8)
			val |= SNES_UP_MASK | SNES_RIGHT_MASK;
*/
...
//	int om_direction = get_direction_controler();
/*	int om_buttons = get_action_buttons(); */
...
//	if (om_buttons) printf("button=%i\n",om_buttons);

/*
	if (om_buttons==2) {
			val |= SNES_SELECT_MASK;
			accelerometer_tare = 1;
	} else if (om_buttons==7) {
			val |= SNES_SELECT_MASK;
			val |= SNES_START_MASK;
	} else if (om_buttons==5) {
			val |= SNES_START_MASK;
	} else if (om_buttons==1) {
			val |= SNES_X_MASK;
	} else if (om_buttons==8) {
			val |= SNES_X_MASK;
			val |= SNES_Y_MASK;
	} else if (om_buttons==3) {
			val |= SNES_Y_MASK;
	} else if (om_buttons==4) {
			val |= SNES_A_MASK;
	} else if (om_buttons==9) {
			val |= SNES_A_MASK;
			val |= SNES_B_MASK;
	} else if (om_buttons==6) {
			val |= SNES_B_MASK;
	}
*/
