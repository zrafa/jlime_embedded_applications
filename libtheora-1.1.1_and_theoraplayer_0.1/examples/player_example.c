...
/* for nn */
  char *buffer=ogg_sync_buffer(oy,256);
  int bytes=fread(buffer,1,256,in);
...
int main(int argc,char *const *argv){
  ...
  _setmode( _fileno( stdin ), _O_BINARY );
	...
	if (Mix_OpenAudio(11025, AUDIO_S16, 1, 1024) < 0) {
		fprintf(stderr, "Warning: Audio could not be setup: %s\n", SDL_GetError());
		exit(1);
	}
