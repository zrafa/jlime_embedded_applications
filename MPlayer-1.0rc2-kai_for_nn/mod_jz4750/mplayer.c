...
static double update_video(int *blit_frame)
{
    ...
    *blit_frame = 0; // Don't blit if we hit EOF
	...
	//printf("%s:%d : frame_dropping=%d\n",__FUNCTION__,__LINE__,frame_dropping);
	if(frame_dropping)
	{
	...
	//update_teletext(sh_video, mpctx->demuxer, 0);
	//update_osd_msg();
	//printf("%s:%d av_drop=%d\n",__FUNCTION__,__LINE__,av_drop);
	if(av_drop == 0)
	{
		current_module = "decode_video";
		//printf("%s:%d\n",__FUNCTION__,__LINE__);
		decoded_frame = decode_video(sh_video, start, in_size, 
					     drop_frame, sh_video->pts);
		current_module = "filter_video";
		//printf("%s:%d\n",__FUNCTION__,__LINE__);
		*blit_frame = (decoded_frame && filter_video(sh_video, 
							     decoded_frame,
							     sh_video->pts));
		//printf("%s:%d\n",__FUNCTION__,__LINE__);
	}
...
#endif
#if 1
  ipu_vbase = mmap((void *)0, IPU__SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, mmapfd, 
		  IPU__OFFSET);
  ipu_maped = 1;
  ipu_enable(ipu_vbase);
  mc_base = mmap((void *)0, MC__SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, mmapfd, 
			MC__OFFSET);
	me_base = mmap((void *)0, ME__SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, mmapfd, 
			ME__OFFSET);

	dblk_base = mmap((void *)0, DBLK__SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, mmapfd, 
			DBLK__OFFSET);
	idct_base = mmap((void *)0, IDCT__SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, mmapfd, 
			IDCT__OFFSET);	
