...
static int FindNextKeyFrameIdx(demuxer_t * demux)
{
	...
	{		
		...
		idx=&((AVIINDEXENTRY *)priv->idx)[idxpos];
		idxpos++;
...
static int demux_avi_read_packet(demuxer_t *demux,demux_stream_t *ds,unsigned int id,unsigned int len,int idxpos,int flags)
{
	...
	if(ds==demux->audio) {
		...
		if(ds==demux->video) {
			...
			if(ds)
				++priv->video_pack_no;
			pts = priv->avi_video_pts = (float)(priv->video_pack_no * per_frametime_ms)/1000.0;
			if(ds) {	
...
do{
  ...
  if(priv->idx_size>0 && priv->idx_pos<priv->idx_size){
    ...
    //printf("idx info 0x%x  :  0x%x  :  0x%x  : 0x%x  : 0x%x\n",idx->ckid,idx->dwFlags,idx->dwChunkOffset,idx->dwChunkLength,flags);
  } else {
    demux->filepos=stream_tell(demux->stream);
...
static demuxer_t* demux_open_avi(demuxer_t* demuxer){
  ...
  per_frametime_ms = sh_video->video.dwScale * 1000 / sh_video->video.dwRate;
  return demuxer;
  ...
  case DEMUXER_CTRL_GET_KEYFRAME_TIME:
	{
			...
			*(float *)arg = ((float)(count * per_frametime_ms) * 0.001);				
			return DEMUXER_CTRL_OK;
	}
	case DEMUXER_CTRL_GET_TIME_LENGTH:
    	    if (!priv->numberofframes || !sh_video) return DEMUXER_CTRL_DONTKNOW;
