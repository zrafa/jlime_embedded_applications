...
while(1) {
  ...
  if(id==mmioFOURCC('L','I','S','T')) {
	  ...
	  if(id==listtypeAVIMOVIE) {
		  ...
		  mp_msg(MSGT_HEADER,MSGL_V,MSGTR_MPDEMUX_AVIHDR_FoundMovieAt,
...
//      printf("\nindex to %p !!!!! (priv=%p)\n",priv->idx,priv);
	...
	entry->dwFlags&=0xffff;
      ...
      if( mp_msg_test(MSGT_HEADER,MSGL_DBG2) ) 
	      print_index(priv->idx,priv->idx_size,MSGL_DBG2);
	    }
	    break;
    /* added May 2002 */
  ...
  if(list_end>0 && chunksize+stream_tell(demuxer->stream)>list_end) {
	  mp_msg(MSGT_HEADER,MSGL_V,
		 MSGTR_MPDEMUX_AVIHDR_BrokenChunk,chunksize,(char *) &id);
