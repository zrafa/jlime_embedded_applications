...
{
    double pts = written_audio_pts(sh_audio, d_audio) - playback_speed *
	audio_out->get_delay();
...
double a_pts = written_audio_pts(mpctx->sh_audio, mpctx->d_audio) - mpctx->delay;
if (a_pts < 0)
	audio_delay -= 3.6f; // vfr problem , immediate value for A320 
...
/*========================== PLAY VIDEO ============================*/
  ...
  if (!mpctx->num_buffered_frames) {
     ...
     if (verbose > 0)
	printf("ftime=%f mpctx->delay = %f\n",frame_time,mpctx->delay);
