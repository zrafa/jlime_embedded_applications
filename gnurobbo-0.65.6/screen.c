...
show_game_area (void)
{
  ...
  if ((viewport.x != vpepx || viewport.y != vpepy)
      ...
      if (viewport.x != vpepx)
	{
	  if (viewport.x < vpepx)
	    {
	      viewport.x +=
		ceil ((float) (vpepx - viewport.x) / viewport.cycles_to_dest);
	    }
	  else
	    {
	      viewport.x -=
		ceil ((float) (viewport.x - vpepx) / viewport.cycles_to_dest);
	    }
	}
      if (viewport.y != vpepy)
	{
	  if (viewport.y < vpepy)
	    {
	      viewport.y +=
		ceil ((float) (vpepy - viewport.y) / viewport.cycles_to_dest);
	    }
	  else
	    {
	      viewport.y -=
		ceil ((float) (viewport.y - vpepy) / viewport.cycles_to_dest);
	    }
	}
  ...
  for (y = viewport.y; y < viewport.y + viewport.h; y++)
	...
	{
	  destrect =
	    set_rect (xoffset + (x - viewport.x) * video.field_size,
...
show_helpscreen (void)
{
  ...
  int iconoffsets1[7 * 2] =
    { 52, 1, 103, 1, 137, 18, 69, 35, 120, 1, 137, 1, 1, 18 };
      ...
      else if (helppage == 1)
	{
	  for (count = 0; count < TXT_HELP_PAGE1_ROWS; count++)
	      ...
	      if (count == 4)
		{
	      ...
	      image =
		TTF_RenderUTF8_Shaded (font,
				       &txt_help_page1[count * TXT_MAX_COLS],
				       fgcolor, bgcolor);
	      destrect =
		set_rect (xoffset + 2 * video.field_size / 16 +
